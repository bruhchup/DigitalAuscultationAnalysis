...
class ConvBlock5x5(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=(1, 1)):
        ...
        self.conv1 = nn.Conv2d(in_channels=in_channels, out_channels=out_channels,
                               kernel_size=(5, 5), stride=stride, padding=(2, 2), bias=False)
        self.bn1 = nn.BatchNorm2d(out_channels)
        self.init_weight()
...
class _CNN6(nn.Module):
    ...
    def forward(self, x):
        ...
        x2 = torch.mean(x, dim=2)  # mean over freq dim
        x = x1 + x2
...
class CNN6(nn.Module):
    def __init__(self, num_classes=4, do_dropout=False, embed_only=False, from_scratch=False,
                 path_to_weights="Cnn6_mAP=0.343.pth", device="cuda"):
        super(CNN6, self).__init__()
        self.cnn6 = _CNN6(num_classes=num_classes, do_dropout=do_dropout, embed_only=embed_only).to(device)
        if not from_scratch:
            weights = torch.load(path_to_weights, map_location=device, weights_only=False)['model']
...
class CNN10(nn.Module):
    def __init__(self, num_classes=4, do_dropout=False, embed_only=False, from_scratch=False,
                 path_to_weights="Cnn10_mAP=0.380.pth", device="cuda"):
...
class CNN14(nn.Module):
    def __init__(self, num_classes=4, do_dropout=False, embed_only=False, from_scratch=False,
                 path_to_weights="Cnn14_mAP=0.431.pth", device="cuda"):
