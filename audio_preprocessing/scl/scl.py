...
def train_epoch_scl(encoder, projector, train_loader, train_transform, criterion, optimizer, scheduler, device):
    epoch_loss = 0.0
...
def train_scl(encoder, projector, train_loader, train_transform, criterion, optimizer, scheduler, epochs, device):
    best_loss = None
    train_losses = []
    ...
    for i in range(1, epochs + 1):
        print(f"Epoch {i}")
        train_loss = train_epoch_scl(encoder, projector, train_loader, train_transform, criterion, optimizer, scheduler, device)
...
def linear_scl(encoder, checkpoint, classifier, train_loader, val_loader, val_transform, criterion, optimizer, epochs, device, num_classes=4):
    best_icbhi_score = 0
    best_se = 0
    best_sp = 0
    best_epoch_icbhi = 0
