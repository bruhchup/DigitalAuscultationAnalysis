...
Every model uses MFCC-derived features exclusively.
  ...
  Classical ML (RF, SVM, GB):  130-dim summary vector
    - 20 MFCCs + delta + delta-delta: mean & std each = 120
    - Spectral centroid, bandwidth, rolloff, ZCR, RMS: mean & std = 10
    
  CNNs (SmallCNN, SpecAugmentCNN):  Full MFCC matrix as 2D input
    - (60 x time_frames x 1) where 60 = 20 MFCCs + 20 delta + 20 delta-delta

All models evaluated on the SAME test samples for fair comparison.

Usage:
  python train_mfcc_comparison.py --data_dir data/ICBHI_final_database

Output: results/mfcc_comparison/
  - Per-approach confusion matrices
  - ROC curves, per-class recall/precision/F1 charts
  - Binary analysis (sensitivity vs specificity)
  - Overfitting comparison, summary dashboard
  - Feature importance (RF, GB), training curves (CNNs)
...
# ============================================================================
# MFCC EXTRACTION (both representations per cycle)
# ============================================================================

def extract_mfcc_vector(audio_pp):
    """130-dim summary for classical ML."""
    feats = []
    mfccs = librosa.feature.mfcc(y=audio_pp, sr=TARGET_SR, n_mfcc=N_MFCC)
    for arr in [mfccs, librosa.feature.delta(mfccs),
...
def extract_mfcc_matrix(audio_pp):
    """(60 x time_frames) matrix for CNN."""
    mfccs = librosa.feature.mfcc(y=audio_pp, sr=TARGET_SR, n_mfcc=N_MFCC,
                                  n_fft=512, hop_length=512)
    delta = librosa.feature.delta(mfccs)
    delta2 = librosa.feature.delta(mfccs, order=2)
    mat = np.concatenate([mfccs, delta, delta2], axis=0).astype(np.float32)
    # z-score normalize
    return (mat - mat.mean()) / (mat.std() + 1e-8)


def build_datasets(cycles):
    """Extract BOTH vector and matrix for each cycle.
    
    Returns aligned arrays so all models evaluate on the exact same samples.
    """
    vectors, matrices, labels = [], [], []
    for c in cycles:
        try:
            pp = preprocess(c["audio"], c["sr"])
            vec = extract_mfcc_vector(pp)
            mat = extract_mfcc_matrix(pp)
...
def feature_names_130():
    n = []
    for s in ["mean","std"]:
        for i in range(20): n.append(f"MFCC_{i+1}_{s}")
    for s in ["mean","std"]:
        for i in range(20): n.append(f"dMFCC_{i+1}_{s}")
    for s in ["mean","std"]:
        for i in range(20): n.append(f"d2MFCC_{i+1}_{s}")
    for f in ["spec_centroid","spec_bandwidth","spec_rolloff","zcr","rms"]:
        n.append(f"{f}_mean"); n.append(f"{f}_std")
    return n
...
def plot_feat_importance(model, name, out):
    if not hasattr(model, "feature_importances_"): return
    fi = model.feature_importances_
    fn = feature_names_130() if len(fi)==130 else [f"f{i}" for i in range(len(fi))]
    top = np.argsort(fi)[-20:]
    fig, ax = plt.subplots(figsize=(10,8))
    ax.barh([fn[i] for i in top], fi[top], color="#3498db")
...
def main():
    ...
    parser.add_argument("--output_dir", default="results/mfcc_comparison")
    ...
    # Extract BOTH representations simultaneously for alignment
    print(f"\n{'='*70}\nEXTRACTING MFCC FEATURES\n{'='*70}")
    print("  Building vectors (130-dim) + matrices (60 x time) per cycle...")
    ...
    for r in results:
        ...
        plot_feat_importance(r.get("model"), r["name"], out)
